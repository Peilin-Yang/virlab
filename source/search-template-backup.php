...
<?php
	...
	$mysqlCol = mysqlConnectDatabase('collections2');
...
<?php
$qry="";
if(isset($_GET["qry"])) $qry = PREG_REPLACE("/[^0-9a-zA-Z ]/i", '', $_GET["qry"]);
...
else
{
	...
	if(isset($_GET["collection"]) && isset($_GET["topic"]))
	{
		...
		$resultArray = readDatabase($mysqlCol,$query);
...
<div id="accordion">
<?php
	$query="select collectionName from collection where indexID = $myIndexID";
	$resultArray=readDatabase($mysqlCol,$query);
	foreach($resultArray as $result)
	{
		$collectionName=$result[0];
		echo "<h3>$collectionName</h3>\n";
		$theQuery = "qry_$collectionName";
		echo "<div>\n";
		$query="select topic,query from $theQuery";

		$resultArray2 = readDatabase($mysqlCol,$query);
